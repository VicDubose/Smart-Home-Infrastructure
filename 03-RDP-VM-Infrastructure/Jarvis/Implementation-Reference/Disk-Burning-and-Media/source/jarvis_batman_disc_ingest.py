...
def main():
    ...
    parser.add_argument(
        "--titles",
        default=None,
        help="Comma-separated explicit DVD title numbers in episode order.",
    )
    ...
    if args.titles:
        ...
        if not requested_titles:
            raise SystemExit("ERROR: --titles was supplied but no title numbers were provided.")
        ...
        unavailable = [
            n for n in requested_titles
            if n not in by_title
        ]

        if unavailable:
            raise SystemExit(
                f"ERROR: Explicit DVD titles not found: {unavailable}"
            )
        ...
        selected = set(requested_titles)

        play_all = [
            t for t in play_all
            if int(t["title"]) not in selected
        ]
        ...
        print(
            "Using DVD titles in episode order: "
